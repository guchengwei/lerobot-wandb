...
def test_inspect_carries_installed_lerobot_checkout_commit(tmp_path, monkeypatch):
    """The metadata seam is independent of this repository's checkout layout."""
    root = _write_empty_dataset(tmp_path / "dataset")
    monkeypatch.setattr(adapter_module, "lerobot_git_commit", lambda: "installed-checkout-head")
    metadata = inspect_dataset_directory(root)
    assert metadata.git_commit == "installed-checkout-head"
